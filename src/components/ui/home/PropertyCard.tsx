...
const PropertyCard: React.FC<PropertyCardProps> = ({ imageSrc, price, name, isPopular, address, bed, bath, size, listingType, listedBy, freq, slug }) => {
            ...
            <div className="flex flex-col gap-4 text-[#002353] px-6 py-6 text-left relative">
                ...
                <div className="flex flex-col gap-[5px]">
                    <h1 className="text-xl md:text-2xl font-extrabold text-[#3871C1] leading-tight">
                        ₱{formatPricePH(price)}
                        {listingType === 'rent' && (
                            <span className="text-base font-normal text-[#002353]/60"> / {freq}</span>
                        )}
                    </h1>
                    ...
                    {/* 👇 Listed by */}
                    {listedBy && (
                        <div className="flex items-center gap-2 mt-2 text-xs md:text-sm text-[#5C7188]">
                            <User className="h-4 w-4 text-[#3871C1]" /> {/* subtle icon */}
                            <span>
                                Listed by <span className="font-semibold text-[#002353]">{listedBy}</span>
                            </span>
                        </div>
                    )}
                </div>
